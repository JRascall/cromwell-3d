 ...
 *
 * WHAT IT IS FOR HERE. The sun's shadow map handles what the sun cannot reach;
 * this handles what the SKY cannot reach. Those are different questions, and
 * without the second one every inside corner in the scene - wall meeting
 * floor, crate meeting ground, the underside of a canopy - receives the full
 * hemisphere as if it were open ground, which is exactly why untextured boxes
 * read as though they were pasted onto the floor rather than standing on it.
 * It is the cheap screen-space stand-in for the contact darkening a baked
 * light probe bake would resolve properly.
 *
 * It multiplies the AMBIENT term only. Occluding direct sunlight with it would
 * be double-counting the shadow map and would darken lit faces that are
 * genuinely lit.
 *
 * Works in VIEW SPACE: position comes back from the depth buffer through the
 * inverse projection, so the depth-compare against neighbouring samples is a
 * comparison of linear distances rather than of a non-linear depth encoding.
 */
...
uniform sampler2D texture0;      /* raylib binds the blit source here; unused */
uniform vec4 colDiffuse;

uniform sampler2D uDepth;
uniform sampler2D uNormals;      /* world normals, n * 0.5 + 0.5 */
...
uniform float uRadius;           /* world units - tiles */
uniform float uBias;
uniform float uStrength;

const int kKernelSize = 24;
uniform vec3 uKernel[kKernelSize];
...
void main()
{
    ...
    for (int i = 0; i < kKernelSize; i++) {
        vec3 samplePosition = position + (basis * uKernel[i]) * uRadius;
    ...
    float ao = 1.0 - (occlusion / float(kKernelSize)) * uStrength;
