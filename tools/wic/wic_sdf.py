    ...
    if pat and dest:
        ...
        for e in ents:
            if fnmatch.fnmatch(e[0].lower(), pat.lower()):
                ...
                # Deliberately NO per-file skip here. Later archives are
                # patches that re-issue earlier files, so extraction is
                # last-writer-wins and must always overwrite; a size-based
                # skip would silently keep a superseded version whenever the
                # two happened to be the same length. Resumability lives at
                # archive granularity in wic_extract.ps1 instead.
