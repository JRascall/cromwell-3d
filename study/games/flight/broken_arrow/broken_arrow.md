...
[`ruse.md`](../../strategy/ruse.md) covers — a modern-warfare RTS with a deep-zoom camera,
...
**Aircraft movement has its own note.**
[`broken_arrow_aircraft.md`](broken_arrow_aircraft.md) answers what §4.2's system
table leaves open — how an aircraft steers, reaches a waypoint accurately enough
to bomb with, and copes with the order changing mid-attack. It is built on a
stronger source than this note: an Il2CppDumper dump of the retail
`GameAssembly.dll`, which yields namespaces, field names, `const` values and the
developers' own Unity tooltips rather than the bare identifier table used here.
The headline is that **aircraft touch neither pathfinder in §5** — they fly a
Dubins path built with the aircraft's own turn radius, and there is no PID
anywhere in the aircraft path.

**Vehicle animation has its own note.**
[`vehicle_animation.md`](../../../topics/animation/vehicle_animation.md) reads the turret, suspension,
...
**Infantry squads have their own note.**
[`broken_arrow_squads.md`](broken_arrow_squads.md) answers how a squad moves,
aims, shoots, is shot at and is drawn — and finds a **three-tier entity model**
where the squad is the unit, the soldier is a body with no health or collider or
target, and **the weapon is a first-class entity** with its own target and reload,
so one squad can engage three enemies at once. The abstraction stays honest
because `UsingAliveSoldierCount` divides the shared weapon's cycle time, so
casualties cost firepower continuously without a man ever being simulated.

**Ground vehicles have their own note.**
[`broken_arrow_vehicles.md`](broken_arrow_vehicles.md) completes the locomotion set
and finds that **the simulation has no concept of a chassis type at all** — no
`IsTracked`, no propulsion enum. Wheeled versus tracked is the *ratio* between
`MaxSpeedRoad` and `MaxCrossCountrySpeed` plus `TurnRate`, and the behaviour
emerges because the pathfinder biases toward roads and the speed selector samples
terrain under the unit. Visually, one `BoneContainer` holds `Wheels`, `Suspension`
and `Tracks` side by side, tracks are a **scrolling material offset** rather than
animated geometry, and the suspension is driven by bounded noise that never
touches the ground.

**Animation has its own note.**
[`broken_arrow_animation.md`](broken_arrow_animation.md) finds **no GPU animation
of any kind** — no vertex or bone animation textures, no compute skinning — and a
simulation→visual channel of **four floats plus twenty events**, feeding
`[SerializeReference] IAnimationBehaviour[]` arrays. **Twenty-five generic
behaviours cover every unit type in the game**: a rotor is `Torque`, landing gear
is `RotateBySpeed`, and audio and VFX are entries in the same list. Physics
reports for the living and simulates only for the dead.

**Vision and target search have their own note.**
[`broken_arrow_vision.md`](broken_arrow_vision.md) answers how a unit finds an
enemy: a **six-byte-per-pixel** baked terrain grid marched with Bresenham off the
main thread, with physics raycasts issued **only where the grid already said
vision was clear**. The march returns a **per-terrain histogram** rather than a
verdict, which is why seeing and shooting are separate. Search cadence is
**membership in an ECS set**, not a timer. And a height-aware march plus
`FOREST_HEIGHT = 25` against a helicopter's 8 m and 40 m altitudes gives
nap-of-the-earth gameplay with **no helicopter-specific vision code at all**.
...
Related: [`ruse.md`](../../strategy/ruse.md) (the bespoke-engine counterpart, and §10 here reads
the two against each other), [`map_scale.md`](../../../topics/scale/map_scale.md),
[`lod_systems.md`](../../../topics/world/lod_systems.md), [`terrain_rendering.md`](../../../topics/world/terrain_rendering.md),
[`valve_networking.md`](../../valve/valve_networking.md),
[`nav_architecture.md`](../../../plans/nav_architecture.md).
...
long-running trade in [`battle_scale.md`](../../../topics/scale/battle_scale.md) — count against depth —
...
[`nav_architecture.md`](../../../plans/nav_architecture.md) argues for: *there is no navigation
...
**And there is a third representation, which strengthens the point.**
[`broken_arrow_aircraft.md`](broken_arrow_aircraft.md) finds that fixed-wing
aircraft use *neither* of these — no graph, no navmesh, no A\* — but a Dubins path
built from turn circles and common tangents, because their binding constraint is
turn radius rather than obstacles, and no graph search expresses that. Three
representations, chosen by what each kind of agent is actually constrained by, is
a considerably better illustration of the argument than two.
...
the mechanism [`valve_networking.md`](../../valve/valve_networking.md) §7.3 documents in full
from TF2's server.
...
[`networked_animation_physics.md`](../../../topics/animation/networked_animation_physics.md) shows is
...
[`nav_architecture.md`](../../../plans/nav_architecture.md) §11 argues for, shipped. The
...
**Related notes**
- [`ruse.md`](../../strategy/ruse.md) — the bespoke-engine counterpart; §10 here is the comparison
- [`nav_architecture.md`](../../../plans/nav_architecture.md) — representations versus algorithms, which §5 is a shipped instance of
- [`valve_networking.md`](../../valve/valve_networking.md) — the replication model §7 belongs to
- [`lod_systems.md`](../../../topics/world/lod_systems.md) — the four meanings of LOD; §3.2 uses three of them
- [`battle_scale.md`](../../../topics/scale/battle_scale.md) — count against depth, which §4.2's per-shell ballistics spends
