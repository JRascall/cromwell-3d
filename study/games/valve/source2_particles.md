...
**[`particle_architecture.md`](../../plans/particle_architecture.md) is the decision this note
...
[`helldivers2_vfx.md`](../shooters/helldivers2/helldivers2_vfx.md) documents the same idea from the other
...
- **Function priority is explicit and order-based** `[VDC]`: where two identical
  ...
  documents for AnimGraph transitions and [`ai_state_machines.md`](../../topics/agents/ai_state_machines.md)
...
| **Godot 4 particles** | MIT | GPUParticles3D: the whole simulation is a *process material* shader, which is a legitimately good design and the closest thing to a modern reference implementation you may legally read and copy. | Not liftable (RenderingServer-coupled) but **the best free reading** on GPU-side particle simulation. Same role Godot plays for IK in [`rigging_ik.md`](../../topics/animation/rigging_ik.md) §6. |
...
- **World queries are a plane, not a world.** Collision arrived in 1.8 and is
  ...
  ([`nav_architecture.md`](../../plans/nav_architecture.md)'s framing).
...
2. **`cromwell` may not call into `game/`** — CLAUDE.md's one architectural rule,
   ...
   [`console_porting.md`](../../plans/console_porting.md) §1.1 argues for elsewhere.
...
- **`src/cromwell/ui/` is a 48-file draw-list widget kit** — sliders, steppers,
  ...
  lines 115–124). *(Note: [`console_porting.md`](../../plans/console_porting.md) §5 states
...
particles.** [`rdr2_atmospherics.md`](../rendering/rdr2_atmospherics.md) is the same move for
fog and clouds, and [`voxel_terrain.md`](../../topics/world/voxel_terrain.md) the same for volume
generally.
