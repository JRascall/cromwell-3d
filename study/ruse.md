...
> [`map_scale.md`](map_scale.md) opens with a warning that Eugen have published
...
[`lod_systems.md`](lod_systems.md), [`battle_scale.md`](battle_scale.md).
...
thinner battlefield than [`battle_scale.md`](battle_scale.md)'s Total War field
...
Contrast [`world_streaming.md`](world_streaming.md) §5, where Epic's
...
[`voxel_terrain.md`](voxel_terrain.md) notes for Space Engineers' chunked
...
so the three vary together. This is [`terrain_rendering.md`](terrain_rendering.md)
...
it is exactly the failure [`source_fps_viewmodel.md`](source_fps_viewmodel.md)
...
**animation has LODs too**, which is [`lod_systems.md`](lod_systems.md)'s
...
same economy [`spatial_queries.md`](spatial_queries.md) §3.6 records for
...
[`map_scale.md`](map_scale.md) §3.3 inferred that R.U.S.E. must hold a
...
[`motion_matching.md`](motion_matching.md) §8.2 and CLAUDE.md's *"when a choice
...
matrix — the same shape [`battle_scale.md`](battle_scale.md) describes for
...
Compare [`valve_networking.md`](valve_networking.md), where the entire
...
That is the lockstep tax [`valve_networking.md`](valve_networking.md) and
[`rainbow_six_siege.md`](rainbow_six_siege.md) both name — change the simulation
...
| File | What it does |
|---|---|
| [`../tools/ruse_edat.py`](../tools/ruse_edat.py) | `edat` v1/v2 archives — header, prefix-tree dictionary, entry extraction |
| [`../tools/ruse_ndf.py`](../tools/ruse_ndf.py) | `EUG0`/`CNDF` — section directory, the four string tables, object and property walk |
| [`../tools/ruse_python.py`](../tools/ruse_python.py) | `.xyz` — zlib, `marshal.loads`, then walk the bytecode pairing `LOAD_CONST` with the following `STORE_NAME` to recover module-level assignments |
...
**Related notes**
- [`map_scale.md`](map_scale.md) — the earlier outside-in note; §1.2 here corrects its scale figures for R.U.S.E.
- [`terrain_rendering.md`](terrain_rendering.md) — anti-tiling, of which the diversity map is the first layer
- [`world_streaming.md`](world_streaming.md) — Unreal's single-representation streaming, and why it needs the mip/LOD clamp R.U.S.E. avoids
- [`spatial_queries.md`](spatial_queries.md) — raycasts as the dominant AI cost, and building a cheap surface to trace against
- [`lod_systems.md`](lod_systems.md) — the four meanings of LOD; R.U.S.E. uses geometry, aggregation and deformation
- [`valve_networking.md`](valve_networking.md) — the authoritative-server alternative to lockstep
- [`motion_matching.md`](motion_matching.md) §8.2 — hysteresis as a bias, against Eugen's time floor
