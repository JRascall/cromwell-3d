...
/* Who is signed in, and the avatar fetched for them.
 *
 * A view model rather than a SteamClient reference: the panel should not be
 * able to start or stop a Steam session, and building this each frame is what
 * keeps it unable to. */
struct DevSteam {
    bool        running = false;
    std::string reason;        /* why not, when not running */
    std::string persona;
    uint64_t    steamId = 0;

    /* The fetch's progress, as words for the panel. */
