...
void PathPreviewBuilder::buildArticulated()
{
...
void PathPreviewBuilder::buildAnchored()
{
    const Lattice& lattice = world_.lattice();
    ...
    for (int index : *route_) {
        ...
        out_->push_back({ static_cast<float>(cell.x) + 1.0f,
                          ...
                          highestFloorUnder(world_, cell, unit_->footprint()),
                          index });
    }
}
                               ...
                               const std::vector<int>& route, std::vector<PathPoint>& out)
{
    ...
    route_ = &route;
    out_   = &out;
    switch (unit.mobility().pathStyle()) {
        case PathStyle::Anchored:    buildAnchored();    break;
        case PathStyle::Articulated: buildArticulated(); break;
    }
}
