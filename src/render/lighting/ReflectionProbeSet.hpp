...
public:
     ...
     * anything that edits the world: destruction merges rooms, and a probe
     * list from before the wall came down describes a building that no longer
     * exists.
     *
     * Placement only — it captures nothing. Every layer is stale afterwards,
     * which is what markAllStale is for. */
    void build(const RoomPartition& rooms, const Lattice& lattice,
