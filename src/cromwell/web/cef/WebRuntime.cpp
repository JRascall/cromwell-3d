...
bool WebRuntime::start()
{
    ...
    const std::filesystem::path helper = root / "cromwell_web_helper.exe";

    if (!std::filesystem::exists(helper)) {
        reason_ = "cromwell_web_helper.exe not found beside the executable";
        return false;
    }
...
void WebRuntime::stop()
{
     ...
     * alive. The visible symptom was nine orphaned cromwell_web_helper.exe
